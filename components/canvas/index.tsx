...
const Canvas = forwardRef<CanvasHandler, CanvasProps>(function Canvas(
	{ imageFile, imageAngle, steps, currentStep, setCurrentStep, resetAngle, setResult },
	ref
) {
	...
	const [distanceInPX, setDistanceInPX] = useState<number>(0);
	const [drawingActions, setDrawingActions] = useState<DrawDetail[]>(() => {
		const saved = localStorage.getItem("drawingActions");
		return saved ? JSON.parse(saved) : [];
	});

	useEffect(() => {
		localStorage.setItem("drawingActions", JSON.stringify(drawingActions, null, 2));
	}, [drawingActions]);
	...
	const handleFrankfortPlane = (drawDetail: DrawDetail) => {
		drawHorizontalLine(canvasContext, fixedWidth, drawDetail);
		setCurrentPath((prev) => [
			...prev,
			{
		...
		drawingActions.forEach((drawDetail) => {
			...
			switch (drawDetail.type) {
				case DrawType.Dot:
					...
					break;
				case DrawType.FrankfortPlane:
					drawHorizontalLine(ctx, fixedWidth, drawDetail);
					break;
				case DrawType.Line:
	...
	const computeGonionAngle = () => {
		const P1Point = findDrawingAction(PointName.P1);
		const P2Point = findDrawingAction(PointName.P2);
		const MeToLrLine = findDrawingAction(PointName.MeToLr);

		if (P1Point && P2Point && MeToLrLine && canvasContext) {
			const intersectPoint = findIntersectPointFromExtendLine(
	...
	const computePalatalPlane = () => {
		...
		if (NPoint && PoPoint && MeToLrLine && ANSPoint && PNSPoint && P2Point && canvasContext) {

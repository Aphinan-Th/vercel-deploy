...
const Canvas = forwardRef<CanvasHandler, CanvasProps>(function Canvas(
	...
	const computeYAxis = () => {
		...
		if (NPoint && SPoint && PoPoint && MeToLrLine && canvasContext) {
			...
			const intersecPoint = findIntersecPointFromExtendLine(
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				const gnPointTest = {
					type: DrawType.Dot,
					startX: intersecPoint.x,
					startY: intersecPoint.y,
				};
				const angleResultTest = getAngleFromPoints(NPoint, SPoint, gnPointTest);
				console.log(`>>> Y axis  ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	};
	...
	const computeFMA = (): number => {
		...
		if (PrPoint && OrPoint && MeToLrLine && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				const pivotPointTest = {
					type: DrawType.Dot,
					startX: intersecPoint.x,
					startY: intersecPoint.y,
				};
				const angleResultTest = getAngleFromPoints(PrPoint, pivotPointTest, MeToLrLine);

				console.log(`>>> FMA  ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const computeGonionAngle = () => {
		...
		if (RamusPlane && MeToLrLine && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				...
				const angleResultTest = getAngleFromPoints(investRamusPoint, pivotPointTest, MeToLrLine);
				console.log(`>>> Gonion ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const computeInterinsialAngle = () => {
		...
		if (A1Point && ArPoint && B1Point && BrPoint && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				...
				const angleResultTest = getAngleFromPoints(ArPoint, pivotPointTest, BrPoint);
				console.log(`>>> InterinsialAngle  ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const compute1ToSN = () => {
		...
		if (SPoint && NPoint && A1Point && ArPoint && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				...
				const angleResultTest = getAngleFromPoints(SPoint, pivotPointTest, ArPoint);
				console.log(`>>> 1\ ToSN  ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const computeFMIA = (): number => {
		...
		if (PrPoint && OrPoint && B1Point && BrPoint && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				...
				const angleResultTest = getAngleFromPoints(OrPoint, pivotPointTest, B1Point);

				console.log(`>>> FMIA  ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const computePalatalPlane = () => {
		...
		if (NPoint && PoPoint && MeToLrLine && ANSPoint && PNSPoint && RamusPlane && canvasContext) {
			const gnPoint = findIntersecPointFromExtendLine(
				{ x: NPoint.startX, y: NPoint.startY },
				{ x: PoPoint.startX, y: PoPoint.startY },
				{ x: MeToLrLine.endX ?? 0, y: MeToLrLine.endY ?? 0 },
				{ x: MeToLrLine.startX, y: MeToLrLine.startY }
			...
			if (gnPoint?.x && gnPoint?.y) {
				...
				if (intersecPoint?.x && intersecPoint?.y) {
					...
					const angleResultTest = getAngleFromPoints(PNSPoint, pivotPointTest, RamusPlane);
					console.log(`>>> computePalatalPlane ${angleResultTest}`);
					return angleResultTest;
				}
			}
		}
	...
	const computeAngleOfCondylarPathVSOccPlane = () => {
		...
		if (PoCPlane && A1Point && MpPoint && canvasContext) {
			...
			if (intersecPoint?.x && intersecPoint?.y) {
				...
				const angleResultTest = getAngleFromPoints(PoCPlane, pivotPointTest, A1Point);
				console.log(`>>> computeAngleOfCondylarPathVSOccPlane ${angleResultTest}`);
				return angleResultTest;
			}
		}
		return 0;
	...
	const findIntersecPointFromExtendLine = (
		lineStart: Point,
		lineEnd: Point,
		lineSecond: Point,
		lineSecondEnd: Point
		...
		while (totalExtendedDistance < 100) {
			...
			if (intersection?.x != null && intersection?.y != null && canvasContext) {
				...
				drawRectangle(canvasContext, newPivot, "newPivot");
				return intersection;
			}
		}
	};
